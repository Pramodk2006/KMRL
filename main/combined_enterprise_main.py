...
class AIDataProcessor:
    ...
    def get_train_status_summary(self):
        """Get train status summary with three exclusive categories: inducted, standby, and maintenance"""
        summary = {
            'total_trains': 0,
            'inducted_trains': 0,
        ...
        # First mark maintenance trains (based on constraints and priority score)
        ...
        # Any remaining unassigned trains go to standby
        ...
        summary['inducted_trains'] = len([t for t in train_states.values() if t == 'inducted'])
        summary['maintenance_trains'] = len([t for t in train_states.values() if t == 'maintenance'])
        summary['standby_trains'] = len([t for t in train_states.values() if t == 'standby'])
        
        return summary

    def get_detailed_train_list(self):
        """Get detailed list of all trains with unique states"""
        train_details = []
            ...
            if isinstance(ineligible, (list, dict)):
                for train in ineligible:
                    ...
                    if train_id in train_states and train_states[train_id]['state'] is None:
                        train_states[train_id] = {'state': 'ineligible', 'data': train}
        ...
        for train in inducted_trains:
            ...
            if train_id in train_states and train_states[train_id]['state'] is None:
                ...
                if priority_score >= 60:
                    train_states[train_id] = {'state': 'ready', 'data': train}
                else:
                    train_states[train_id] = {'state': 'maintenance', 'data': train}
        
        # Finally mark standby trains
        standby_trains = results.get('standby_trains', [])
        for train in standby_trains:
            train_id = train.get('train_id')
            if train_id in train_states and train_states[train_id]['state'] is None:
                train_states[train_id] = {'state': 'standby', 'data': train}
        
        # Create the detailed list
        rank = 1
        # First add inducted trains (ready and maintenance) to preserve ranking
        for state in ['ready', 'maintenance']:
            for train_id, info in train_states.items():
                if info['state'] == state:
                    ...
                    train_details.append({
                        ...
                        'inducted': state in ['ready', 'maintenance']
                    })
                    rank += 1
        
        # Then add other trains
        for state in ['standby', 'ineligible']:
            for train_id, info in train_states.items():
                if info['state'] == state:
                    train = info['data']
                    train_details.append({
                        'rank': 0,  # Non-inducted trains don't get a rank
                        'train_id': train_id,
                        'status': state.capitalize(),
                        'bay_assignment': train.get('assigned_bay', 'N/A'),
                        'priority_score': train.get('priority_score', 0.0),
